...
export class HtfLiquiditySweepStrategy {
    ...
    execute(ctx) {
        ...
        if (last.volume <= indicators.volumeSma * 1.4)
            return null;
...
export class HtfBreakoutFailureStrategy {
    ...
    execute(ctx) {
        ...
        if (last.volume < indicators.volumeSma * 1.6)
            return null;
...
export class HtfVwapReversionStrategy {
    ...
    execute(ctx) {
        ...
        if (deviationAbs < minDeviation || deviationAbs > maxDeviation)
            return null;
        ...
        if (anchorSlopePct > 0.5)
            return null; // drifting anchor — no mean-reversion setup
