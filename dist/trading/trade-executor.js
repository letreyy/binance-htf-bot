...
export class TradeExecutor {
    exchange = null;
    isLive = config.binance.isLiveMode;
    activeTrades = [];
    todaysPnlPercent = 0;
    strategyStats = {};
    disabledStrategies = new Set();
    slCooldown = new Map();
    targetRiskPercent = config.bot.targetRiskPercent;
...
${pendingOrders.map(t => `- <b>${t.symbol}</b> ${t.direction} (Limit: ${t.entryPrice.toFixed(4)})`).join('\n') || '<i>None</i>'}`;
        ...
        telegramNotifier.onCommand(/\/toggle (.+)/, (_msg, match) => {
        ...
        telegramNotifier.onCommand(/\/leverage fixed (\d+)/, (_msg, match) => {
        ...
        telegramNotifier.onCommand(/\/leverage dynamic(?:\s+(\d+)\s+(\d+))?/, (_msg, match) => {
        ...
        telegramNotifier.onCommand(/(?:\/risk|🛡️ Риск)(?:\s+([\d.]+))?/, (_msg, match) => {
    ...
    isStrategyDisabled(strategyName) {
        if (this.disabledStrategies.has(strategyName))
            return true;
        return statsService.isPaused(strategyName);
    }
    isOnSlCooldown(symbol, strategyName) {
    ...
    calculateLeverage(slDistancePercent) {
    ...
    recordStrategyResult(strategyName, totalPnl) {
    ...
    async calculateLivePositionSize(signal) {
        if (!this.exchange)
            return 0;
        try {
