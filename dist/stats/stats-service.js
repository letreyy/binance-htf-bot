...
export class StatsService {
    ...
    checkUnlock(strategyName) {
        ...
        if (thresh.unlockPF) {
            ...
            if (pf < thresh.unlockPF)
                return false;
        }
        return true;
    }
    evaluatePauses(strategyName) {
        const thresh = DEFAULT_THRESHOLDS[strategyName];
        if (!thresh)
            return;
        const now = Date.now();
        ...
        if (recentStreak.length === STREAK_LOSS_COUNT &&
        ...
        if (recent24h.length < thresh.minN24)
            return;
    ...
    applyPause(strategyName, type, reason) {
        if (this.activePauses.has(strategyName))
            return;
        const thresh = DEFAULT_THRESHOLDS[strategyName];
