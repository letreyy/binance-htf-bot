...
export class ScoringEngine {
    static calculate(ctx, candidate) {
        ...
        if (ctx.funding) {
            ...
            if (ctx.funding.rate > 0.0005 && candidate.direction === SignalDirection.SHORT)
                score += 7;
            if (ctx.funding.rate < -0.0005 && candidate.direction === SignalDirection.LONG)
                score += 7;
            // Normal-range funding alignment
            if (ctx.funding.rate > 0.0001 && candidate.direction === SignalDirection.SHORT)
                score += 3;
            if (ctx.funding.rate < -0.0001 && candidate.direction === SignalDirection.LONG)
                score += 3;
        }
        ...
        if (ctx.candles4h && ctx.candles4h.length > 0 && isTrendFollow) {
            ...
            if ((candidate.direction === SignalDirection.LONG && htfBullish) ||
        ...
        if (ctx.btcContext) {
            if (candidate.direction === SignalDirection.LONG && ctx.btcContext.trend === 'BEARISH')
                return { score: 0, label: ConfidenceLabel.IGNORE };
            if (candidate.direction === SignalDirection.SHORT && ctx.btcContext.trend === 'BULLISH')
                return { score: 0, label: ConfidenceLabel.IGNORE };
        }
