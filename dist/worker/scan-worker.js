        ...
        for (const symbol of topSymbols) {
            ...
            try {
                ...
                for (const strategy of strategyRegistry) {
                    if (tradeExecutor.isStrategyDisabled(strategy.name))
                        continue;
                    if (!TimeFilters.isStrategyAllowed(strategy.id, currentSession))
                        continue;
                    if (tradeExecutor.isOnSlCooldown(symbol, strategy.name))
                        continue;
                    const candidate = strategy.execute(ctx);
                    if (candidate) {
                        if (!passesDirectionFilter(ctx, candidate.direction, candidate.strategyName))
                            continue;
